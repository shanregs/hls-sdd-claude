...
All roles other than the System Assistant/Admin need the mobile app; the web application is used by Director, Managers and the System Assistant from laptop, tablet or desktop. Role-based access control (RBAC) must restrict a Manager to their assigned scope, while the Director and System Assistant see the full organization.

**Manager scope is Zone-based, not a flat list of schools/teachers** (added 2026-09-22, correcting the original independent-assignment model — see Section 7): the "Area" in "Area Coordinator" is a real operational concept, a **Zone** — a geographic grouping of Schools. Each Zone is assigned one or more Managers (a second Manager is added once a Zone has enough Schools that one person can't reasonably cover them all), and each School belongs to exactly one Zone and is managed by one of that Zone's assigned Managers. A Teacher's accountable Manager is **not** a separate assignment — it is whichever Manager currently manages the School the Teacher is contracted to. (Until the system has a real Teacher–School contract to derive this from — Section 10's future `Contract`/SchoolBilling work — an interim direct Teacher-to-Manager assignment stands in, documented as temporary.)
...
- **Zone** — id, name (a geographic area, e.g. "North Chennai"); the grouping every School belongs to and every Manager assignment runs against (added 2026-09-22, Section 2).
- **Teacher** — id, name, contact, bank details, current school assignment, HLS-offered salary, status (in training / active / on leave / exited). Accountable Manager is derived from the current school assignment's Manager (Section 2), not stored independently once a real school assignment exists.
- **School** — id, name/branch, zone, contracted rate per teacher, billing contact, payment mode preference, assigned teachers, assigned manager (must be one of the School's Zone's assigned Managers).
...
**Backend:** Java (25+) with Spring Boot (4.1.1-based), organized as a **modular monolith** — one deployable Spring Boot application with hard package boundaries per bounded context (`identity`, `organization`, `teacher`, `school`, `schoolbilling`, `attendance`, `payroll`, `expense`, `training`, `substitution`, `recruitment` with a `marketing` sub-package, `reporting`, `audit`). `teacher` and `school` each own their own master data; `schoolbilling` owns the Teacher–School–Manager Contract plus receivables and reads Teacher/School only through their public APIs; `organization` owns Manager records and Manager–School scoping; `reporting` is read-only across other modules; `audit` is the single append-only store other modules write history through. Cross-module calls happen only through a module's public interface, enforced by ArchUnit/Spring Modulith rules that fail the build on violation (e.g. attendance finalization triggering payroll calculation happens via an in-process call to payroll's public interface, not a message broker). No microservices by default — extraction of a module into its own service is deferred until scale or team size actually justifies it.
...
- **Resolved 2026-09-22**: Manager scoping was originally modeled (and implemented, in the `organization` module) as two independent assignments — Manager↔School and Manager↔Teacher — with no Zone concept. Clarified that this doesn't match how HLS actually operates: Managers cover geographic Zones (the "Area" in "Area Coordinator"), Schools belong to a Zone and are managed by one of that Zone's Managers, and a Teacher's accountable Manager follows their School's Manager rather than being assigned separately. Section 2 and Section 10 above are corrected accordingly; the already-shipped `organization` module's independent Teacher↔Manager assignment is retained only as a documented, temporary interim mechanism until a real Teacher–School contract (Section 10's `Contract` entity, future SchoolBilling module) exists to derive it for real — see `.specify/memory/constitution.md` Amendment 1.6.0.
- **Resolved 2026-09-22 (later same day)**: Zone was initially placed under the `organization` module's ownership (Amendment 1.6.0). Corrected to belong to `school` (School Master Data) instead — a Zone groups Schools, so it's School data, not Manager data. `school` now owns `Zone(id, name)` and each School's current Zone, built as a minimal slice pulled forward ahead of the rest of School Master Data (Section 10's `School` entity fields); `organization` keeps Manager↔Zone assignment and the Manager-must-be-in-Zone rule, reading Zone data through `school`'s public interface rather than owning it — see `.specify/memory/constitution.md` Amendment 1.7.0.

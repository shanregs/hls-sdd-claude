...
The system is one deployable Spring Boot application organized into packages per bounded context (identity, roster, schoolbilling, attendance, payroll, expense, training, substitution, recruitment, marketing). The `roster` package owns Teacher master data and the Teacher–School–Manager Contract (the reference data that attendance, payroll, school billing, and substitution all transact against); `schoolbilling` owns School master data in addition to receivables, since a school's contracted rate and billing terms are intrinsic to billing. Cross-module calls happen only through a module's public interface; ArchUnit rules enforcing this must fail the build on violation. A module may only be extracted into its own service later if scale or team size actually justifies it — this constitution does not permit microservices by default.
...
**Version**: 1.4.0 | **Ratified**: 2026-09-18 | **Last Amended**: 2026-09-21

...
The system is one deployable Spring Boot application organized into packages per bounded context (attendance, payroll, schoolbilling, expense, training, substitution, recruitment, marketing, identity). Cross-module calls happen only through a module's public interface; ArchUnit rules enforcing this must fail the build on violation. A module may only be extracted into its own service later if scale or team size actually justifies it — this constitution does not permit microservices by default.
...
**Version**: 1.3.0 | **Ratified**: 2026-09-18 | **Last Amended**: 2026-09-21
